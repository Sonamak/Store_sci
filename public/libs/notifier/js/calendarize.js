...
function Calendarize() {
    ...
    return {
        ...
        buildYearCalendar: function(el, year, attendanceData, callback) {
            var _this = this;
            ...
            months.forEach(function(a, b) {
                var $monthNode = _this.buildMonth(b, year, opts, attendanceData);
        ...
        buildMonth: function(monthNum, year, opts, attendanceData) {
            ...
            daysInMonth.forEach(function(c, d) {
                ...
                $dayNode.setAttribute("data-date", moment(c).format('L'));

                // Mark present or absent
                attendanceData.forEach(item => {
                    if (moment(item.date).format('L') === moment(c).format('L')) {
                        if (item.is_present) {
                            $dayNode.classList.add('present');
                        } else {
                            $dayNode.classList.add('absent');
                        }
                    }
                });
                ...
                if (dateParsed === todayParsed) $dayNode.classList.add('today', 'animate-pulse');
                if (dateParsed > todayParsed) $dayNode.classList.add('future');
                if (dateParsed < todayParsed) $dayNode.classList.add('past');

                // Disable Sunday and Saturday as weekend
                if (dow === 0 || dow === 6) $dayNode.classList.add('weekend');

                // Add weekday class for better js interactions
                if (dow !== 0 || dow !== 6) $dayNode.classList.add('weekday');
