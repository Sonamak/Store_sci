        ...
        <div class="max-w-7xl mx-auto px-0 sm:px-4">
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
                ...
                <div class="p-2">
                    ...
                    <form
                        method="POST"
                        action="{{ url('/api/users/' . $user->id . '/update') }}"
                        id="update-user-form"
                        x-data="{ phoneCode: '+0' }"
                    >
                        @csrf
                        ...
                        <div>
                            ...
                            <x-jet-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ $user->name ?? null }}" required autofocus autocomplete="name" />
                        </div>

                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">

                            {{-- Country custom dropdown --}}
                            <x-custom-country-select :id="'country'" :name="'country'" :countries="$countries" :required="true" width="w-full" :selected="$user['country']" />
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($states as $row)
                                        <option value="{{ $row->iso2 }}" {{ $user->state == $row->iso2 ? 'selected' : '' }}>{{ $row->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($cities as $row)
                                        <option value="{{ $row->name }}" {{ $user->city == $row->name ? 'selected' : '' }}>{{ $row->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="locale" id="locale" class="cursor-pointer border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" required>
                                    <option value="ar_AR" {{ $user->locale == 'ar_AR' ? 'selected' : '' }}>عربي</option>
                                    <option value="en_US" {{ $user->locale == 'en_US' ? 'selected' : '' }}>English</option>
                                </select>
                            </div>
                        </div>     
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-jet-input id="email" class="block mt-1 w-full" type="email" name="email" value="{{ $user->email ?? null }}" required />
                            </div>

                            <div>
                                <x-jet-label for="phone" value="{{ translate('dashboard.users.phone') }}" />
                                <x-jet-input id="phone" class="block mt-1 w-full rounded-e-md" type="tel" name="phone" value="{{ $user->phone ?? null }}" required />
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select name="educational_attainment_id" id="educational_attainment" class="cursor-pointer border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" required>
                                ...
                                @foreach($educational_attainment as $row)
                                    <option value="{{ $row->id }}" {{ $user->educational_attainment_id == $row->id ? 'selected' : '' }}>{{ $row->label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select name="general_specialization_id" id="general_specialization" class="cursor-pointer border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" required>
                                ...
                                @foreach($general_specialization as $row)
                                    <option value="{{ $row->id }}" {{ $user->general_specialization_id == $row->id ? 'selected' : '' }}>{{ $row->label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select name="specialization_id" id="specialization" class="cursor-pointer border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm block mt-1 w-full" required>
                                ...
                                @foreach($specialization as $row)
                                    <option value="{{ $row->id }}" {{ $user->specialization_id == $row->id ? 'selected' : '' }}>{{ $row->label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <x-jet-label for="password" value="{{ translate('dashboard.users.password') . ' (' . translate('dashboard.users.leave_blank_to_keep_it_unchanged') . ')' }}" />
                                <x-jet-input id="password" class="block mt-1 w-full" type="password" name="password" autocomplete="password" />
                            </div>
                        </div>
